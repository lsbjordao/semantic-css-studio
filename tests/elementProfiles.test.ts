...
import { propertyGroupsForElement, targetList } from '../src/editor/elementProfiles'

describe('element-specific property profiles', () => {
  it('exposes the blockquote inline-start border controls', () => {
    const properties = propertyGroupsForElement('blockquote').flatMap((group) => group.properties)
    expect(properties.some((property) => property.property === 'borderInlineStartColor')).toBe(true)
    expect(properties.some((property) => property.property === 'borderInlineStartWidth')).toBe(true)
  })
  ...
  it('maps progress value color to cross-browser targets', () => {
    ...
    expect(valueColor && targetList('progress', valueColor).map((target) => target.selector)).toEqual([
      'progress',
