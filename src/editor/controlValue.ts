...
type ShorthandPart =
  'top' | 'right' | 'bottom' | 'left' | 'width' | 'style' | 'color' | 'value'
...
function boxPart(
  value: string,
  part: Extract<ShorthandPart, 'top' | 'right' | 'bottom' | 'left'>,
): string {
...
const borderWidthPattern =
  /^(?:0|thin|medium|thick|-?(?:\d+\.?\d*|\.\d+)(?:px|rem|em|ch|%|vh|vw|vmin|vmax|pt|pc|cm|mm|in|ex)?)$/i
const functionalLengthPattern = /^(?:calc|min|max|clamp)\(/i

function borderPart(
  value: string,
  part: Extract<ShorthandPart, 'width' | 'style' | 'color'>,
): string {
  const tokens = splitTopLevel(value)
  const width =
    tokens.find(
      (token) =>
        borderWidthPattern.test(token) || functionalLengthPattern.test(token),
    ) ?? ''
  const style =
    tokens.find((token) => borderStyles.has(token.toLowerCase())) ?? ''

  if (part === 'width') return width || value
  if (part === 'style') return style || value

  const colorTokens = tokens.filter(
    (token) => token !== width && token !== style,
  )
