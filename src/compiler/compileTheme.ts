...
/**
 * `layers.print` is optional: until a print rule is enabled the theme emits
 * nothing, so older themes and preset snapshots compile byte-identically. When
 * present, the rules go inside `@media print` in a `print` layer. The layer
 * name joins the declaration only in that case and, because it is first used
 * after `responsive`, it sits last in the cascade.
 */
function printRules(
  theme: Theme,
  scopes: readonly string[] = [],
  ...
  const print = theme.layers.print
  if (!print || Object.keys(print).length === 0) return []
  const body = Object.entries(print)
    .map(([selector, styles]) => {
      const inner = declarations(styles, '    ')
      const scoped = scopeSelector(selector, scopes, directBody)
      return inner ? `  ${scoped} {\n${inner}\n  }` : ''
...
export function compileTheme(
  ...
  const print = printRules(theme, scopes, directBody)
  const layerNames = print.length ? [...LAYER_ORDER, 'print'] : LAYER_ORDER
  const blocks = [
